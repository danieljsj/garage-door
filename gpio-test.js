...
setEdge398(later) { [Error: EACCES: permission denied, open '/sys/class/gpio/gpio4/edge']
  ...
  code: 'EACCES',
  syscall: 'open',
  path: '/sys/class/gpio/gpio4/edge' } 4
/home/pi/apps/garage-door/gpio-test.js:33
		if (err) throw err;

Basically it's a permissions thing; a lot of the documentation is for Raspbian which is kind of annoying since I am on MATE... but it's good to have gotten down to it.

here's a ref:  https://github.com/raspberrypi/linux/issues/1117
...
function Gpio() {
    ...
    this.setup = function(channel, direction, edge, onSetup /*err*/) {
        ...
        async.waterfall([
            setRaspberryVersion,
            function(next) {  
                pinForSetup = getPinForCurrentMode(channel); console.log("setup167", pinForSetup);
                if (!pinForSetup) {
            ...
            function(next) {
                exportPin(pinForSetup, next); console.log("setup181", pinForSetup);
            },
            function(next) { console.log("setup183", pinForSetup, edge);
                setEdge(pinForSetup, edge, next);
            },
            function(next) { console.log("setup186", pinForSetup);
...
function setEdge(pin, edge, cb) { console.log(PATH);
    debug('set edge %s on pin %d', edge.toUpperCase(), pin);
    fs.writeFile(PATH + '/gpio' + pin + '/edge', edge, function(err) {
        console.log("setEdge398(later)", err, pin); if (cb) return cb(err); 
    }); console.log("setEdge399", pin);
}
